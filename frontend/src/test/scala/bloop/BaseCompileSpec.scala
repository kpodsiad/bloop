...
abstract class BaseCompileSpec extends bloop.testing.BaseSuite {
  ...
  test("cancel slow compilation") {
    ...
    BuildUtil.testSlowBuild(logger) { build =>
      ...
      println("macro project compiled")
      ...
      val compiledUserState = {
        ...
        try Await.result(backgroundCompiledUserState, Duration(2950, "ms"))
          ...
          case NonFatal(t) => println(t); backgroundCompiledUserState.cancel(); throw t
          case e: InterruptedException =>
            println(e); backgroundCompiledUserState.cancel(); compiledMacrosState
        }
      }
