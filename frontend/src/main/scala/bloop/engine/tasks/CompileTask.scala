    ...
    def compile(graphInputs: CompileGraph.Inputs): Task[ResultBundle] = {
      ...
      bundle.prepareSourcesAndInstance match {
        ...
        case Right(CompileSourcesAndInstance(sources, instance, javaOnly)) =>
          ...
          waitOnReadClassesDir
            .doOnCancel(
              Task(
                println(s"task cancelled compile task 0 ${Thread.currentThread().getName()}")
              )
            )
            .flatMap { _ =>
              ...
              inputs
                .doOnCancel(
                  Task(
                    println(s"task cancelled compile task 1 ${Thread.currentThread().getName()}")
                  )
                )
                .flatMap(inputs => Compiler.compile(inputs))
                .doOnCancel(
                  Task(println(s"task cancelled compileTask 2 ${Thread.currentThread().getName()}"))
                )
                .map { result =>
            ...
            .doOnCancel(
              Task(println(s"task cancelled compile task 3 ${Thread.currentThread().getName()}"))
            )
      }
    }
