...
final class BloopBspServices(
    ...
    def inferDebuggeeRunner(
      ...
      params.dataKind match {
        ...
        case bsp.DebugSessionParamsDataKind.ScalaTestSuites =>
          convert[List[String]](
            filters => {
              BloopDebuggeeRunner.forTestSuite(projects, filters, Map.empty, state, ioScheduler)
            }
          )
        case "scala-test-selection" =>
          convert[ScalaTestSelection](
            testSelection => {
              ...
              val selectedTests = testSelection.classes.collect {
                case ScalaTestSuiteSelection(className, head :: tail) =>
                  (className, NonEmptyList(head, tail))
              }.toMap
              BloopDebuggeeRunner.forTestSuite(projects, filters, selectedTests, state, ioScheduler)
            }
          )
  ...
  def test(params: bsp.TestParams): BspEndpointResponse[bsp.TestResult] = {
    def test(
      ...
      Tasks.test(state, List(project), Nil, testFilter, Map.empty, handler, mode = RunMode.Normal)
    }
